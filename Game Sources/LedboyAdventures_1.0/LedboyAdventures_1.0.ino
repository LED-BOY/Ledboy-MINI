...
void game() {
  ...
  while (lives > 0) {
    ...
    if ((interruptTimer - timer) > 50) { // updates every 50 milliseconds
      uint8_t playerSpriteEnlarge = 0; // enlarge the player sprite area no visible prevents the soild animation to draw over the player
      ...
      if (playerYPos < 2) { // player sprite fordware movement while jumping
        ...
        playerXPos--;
        playerSpriteEnlarge = 3; // when player is returning to positon soild has to be prevented for drawing over player
      }
      ...
      for (uint8_t x = 0; x < 127; x += 8) { // draw trees and soild
        ...
        if (x < (playerXPos - playerSpriteEnlarge) || x > ((playerXPos + 7) - (playerSpriteEnlarge / playerSpriteEnlarge)) ||  playerYPos < 1) {// soild animation/ interaction with player sprite.
    ...
    for (uint8_t x = 0; x < 3; x ++) {// check enemies colition with ledboy to end game
      if ((enemiesPos [x] > (playerXPos - 7) && enemiesPos [x] < (playerXPos + 7) && (playerYPos + 1) == 3) || (enemiesPos [x] > (playerXPos + 120) && enemiesPos [x] < (playerXPos + 128) && (playerYPos + 1) == 3)) {
        lives = 0;
      }
    }
  }
  ...
  oled.clearScreen();// show score based on a  line
...
void drawTitle (uint8_t column, uint8_t page, uint8_t sprite[], uint8_t bitShift, uint8_t lenght) {
  oled.setCursor(column, page);// position cursor column - page
  oled.ssd1306_send_data_start();
