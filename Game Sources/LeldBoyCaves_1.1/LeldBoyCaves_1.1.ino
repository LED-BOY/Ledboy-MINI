...
void game() {
    ...
    while (!endGame) {
        ...
        // Handle cave drops (player fire logic)
        if ((interruptTimer - cavesDropsTimer) > 103) {
        ...
        if (caveDropXPos > playerXPos && caveDropXPos < (playerXPos + 8) && caveDropYPos == 3) endGame = true;

        // Handle cave collapse
        if ((interruptTimer - caveCollapseTimer) > caveCollapseSpeed) {
            caveCollapseTimer = interruptTimer;
            caveCollapseSpeed = 150;
            ...
            if (caveYPos > 1 && caveYPosBitShiftLeft > 3 && !endGame) {
                ...
                while (caveYPosBitShiftLeft < 8) {
                    if (interruptTimer % 40 == 0) {
                        movePlayer((shakePlayer * 2)); // Take out control of the player while cave is resetting
                        shakePlayer = !shakePlayer;
                ...
                caveYPosBitShiftLeft = 0;
                caveYPosBitShiftRight = 8;
                caveCollapseSpeed = 1000 * (randomNumber / 8);
                caveCollapse = false;
                caveYPosBitShiftLeft = 0;
                caveYPos = 1;
            } else {
...
void movePlayer(int8_t playerDirection) {
    ...
    if (playerDirection == 0) playerXPos--; // If value is 0, it is the same as -1 (go left)
...
void intro() {
    ...
    voltageReading();
    oled.drawLine(10, 0, actualVoltage, 0b00111100); // Draw voltage meter
...
void drawTitle (uint8_t column, uint8_t page, const uint8_t* sprite, uint8_t bitShift) {
...
void drawSprite (uint8_t column, uint8_t page, const uint8_t* sprite, bool digit) {
  oled.setCursor(column, page);// position cursor column - page
  oled.ssd1306_send_data_start();
