  ...
  while (setOptions) {
    ...
    if ((interruptTimer - timer) > 2000) {
      ...
      switch (option) {
        ...
        case 2: totalSeconds = (setNumber((option - 1), 5, 45) * 60); break; // Set timer
        case 3: setTime(); break; // Set time
        case 4: interruptSensitivity = setNumber((option - 1), 1, 3) * 5; break;// Set interrupt sensitivity to one of three states: 0(off), 1(hi), 2(med), 3(low)
      }
      setOptions = false;
    }
  }
