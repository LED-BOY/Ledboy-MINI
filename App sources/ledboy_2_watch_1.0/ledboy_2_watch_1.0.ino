    ...
    if (buttonDebounce(250) || optionSelected) {
      ...
      switch (ledColor) { // option select menu.
        case 0:
          ...
          break;
        case 1:
          PORTA.DIR = (1 << 7);// pin2 (REDLED) as output
          break;
        case 2:
          PORTA.DIR = (1 << 6) | (1 << 5); //blue and green led on
          break;
      }
    }
  }
...
void wakeUpFromSleep(bool activateScreen) {
  ...
  while ((interruptTimer - timer) < 300 );//whait some time before initializing screen
...
void options (void) { // options menu
  ...
  oled.drawLine(10, 3, interruptSensitivity * 2, 0B11110000);
  option = 0;
    ...
    if ((interruptTimer - timer) > 2000) {
      ...
      switch (option) { // option select menu.
        case 1:// led on
          ...
          break;
        case 2:// timer
          totalSeconds = (setNumber((option - 1), 5, 30) * 60); // set minutes for timer and multiply to get seconds
