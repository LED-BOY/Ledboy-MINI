/*
  LEDBOY MINI Watch 3.1 for ATtiny series 1.
...
constexpr uint8_t RESET_CANCEL = 0;
constexpr uint8_t RESET_CONFIRM = 1;
...
void loop() {
  ...
  if (minuteTimer.isFinished() && minuteTimer.isActive() && buttonLow()) {
    ...
    optionsDebounce = 800;
    return;  // do not let the same press immediately open the options menu
  }
    ...
    if (buttonDebounce(50) || optionSelected) {
      ...
      if (ledColor) {
        const uint8_t selectedPin =
          ledColor == 1 ? RED_LED_PIN :
          ledColor == 2 ? GREEN_LED_PIN : BLUE_LED_PIN;

        PORTA.OUTCLR = selectedPin;
        PORTA.DIRSET = selectedPin;
      }
    }
  }
}
  ...
  while (true) {
    ...
    if ((uint16_t)(interruptTimer - timer) > 2000) {
      ...
      switch (option) {
        ...
        case 5:
          // Explicit confirmation: 0 cancels, 1 resets the watch.
          if (setNumber(4, 1, 2) == RESET_CONFIRM) {
            _PROTECTED_WRITE(
              RSTCTRL.SWRR,
              1);
          }
