...
#define OLED_RST PORTC.OUT |= (0 << 0);//  resets the oled screen.
#define OLED_ON PORTC.OUT |= (1 << 0);// oled on
...
void setup() {
  ...
  // reset hi score
  while (BUTTONLOW || interruptTimer < 500) {
  ...
  OLED_ON// start oled screen
  oled.begin();// start oled screen
  PORTA.DIR = (BATTCHR << 2);// pin3 (LED) as output takes batchr as source
}
...
void introScreen (void) {
  ...
  PORTC.PIN2CTRL  |= PORT_ISC_RISING_gc; //attach interrupt to portC pin 2 keeps pull up enabled (sensor)
}
...
void game (void) {
  ...
  while (true) {
    ...
    if (soundStatus != sound) {//show  sound on or off
      soundStatus = sound;
      if (sound) {
        drawSprite (4, 3, letterS);// letters S sound on
      } else {
        drawSprite (4, 3, letterM);// letters M mute sound off
      }
    }
      ...
      for ( uint8_t x = 0; x < sizeof (enemiesPattern) / sizeof (uint8_t); x++) {// defines enemies positions.
        ...
        if (!enemyIsDead) {// only if the enemy isn't dead continue.
          ...
          if (playerFirePosX > (enemiesPattern [x] + enemiesPosX) && playerFirePosX < (enemiesPattern [x] + enemiesPosX) + 8 && playerFirePosY == enemiesPosY) { // if palyer hits an enemy
            ...
            if (score += 2, score > 9999) score = 9999;
            showScore(score);
...
void showScore(uint16_t score) {// draws all ledInvadersScore using showTime function
  ...
  if (score < 100) { // functions to draw units in digit 3, decimals in digit 2 and hundredths in digit 1
    ...
    score /= 100;
    drawDecimal (112, 0, score);
    score *= 100;
    valueDecimals -= score;
    drawDecimal (112, 1, valueDecimals);
  }
}
...
void goToSleep (void) {
  PORTA.PIN5CTRL  |= PORT_ISC_BOTHEDGES_gc; //attach interrupt to portA pin 3 keeps pull up enabled
...
uint8_t voltageReading(void) {
  actualVoltage = map(readSupplyVoltage(), MINVOLTAGE, MAXVOLTAGE, 0, 100);
...
ISR(PORTC_PORT_vect) {// turn sound off or on by shaking the device
  ...
  if (interruptDebounce++, interruptDebounce > 2) {// interrupt debounce
    interruptDebounce = 0;
    sound = !sound;
  }
}
